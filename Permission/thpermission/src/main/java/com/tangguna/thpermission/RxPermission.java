...
import com.tangguna.thpermission.handle.Handle;
import com.tangguna.thpermission.permissions.Permission;
import com.tangguna.thpermission.view.RequestFragment;
 ...
 * <p>
 * Describe:Android6.0以上权限使用入口类，适配Android 6.0以上动态权限申请
 */
public class RxPermission {
     ...
     */
    public RxPermission(@NonNull Activity activity, @NonNull String[] permissions, String defaultTips) {
     ...
     */
    public RxPermission(@NonNull Activity activity, @NonNull String... permissions) {
     ...
     * @param defaultTips 为没有重写{@link #onTips}时的默认值
     */
    public RxPermission(@Nullable Activity activity, @NonNull Context context, @NonNull String[] permissions, String defaultTips) {
     ...
     * 只当activity可能传入null 或 为null时（仅仅检测权限） 才可以选择使用本方法，因为activity存在时context参数不会生效，等价于没有context参数的构造函数。
     */
    public RxPermission(@Nullable Activity activity, @NonNull Context context, @NonNull String... permissions) {
        init(activity, context, permissions, "");
    }


    /**
     * 授权完成时回调，会在Success和Failure之一回调后调用。
     *
     * @param resolvePermissions 已授权的权限列表
     * @param lowerPermissions   参考{@link #onPermissionSuccess}，为resolvePermissions的子集
     */
    protected void onComplete(@NonNull ArrayList<String> resolvePermissions, @NonNull ArrayList<String> lowerPermissions, @NonNull ArrayList<String> rejectFinalPermissions, @NonNull ArrayList<String> rejectPermissions, @NonNull ArrayList<String> invalidPermissions) {
        //可选自行实现 完成回调，可能全部已授权、全部未授权、或者部分已授权
        //要么仅仅实现下面这个方法，不管授权了几个权限都会调用一次
    }

    /**
     * 已授权时回调，不管什么情况，Success和Failure肯定有一个会回调。注意：跟API版本有关的方法调用，应自行判断API版本，低版本API中调用高版本权限全部会放行。
     *
    ...
    /**
     * 未授权时回调，不管什么情况，Success和Failure肯定有一个会回调
     *
     ...
     */
    protected void onPermissionFailure(@NonNull ArrayList<String> rejectFinalPermissions, @NonNull ArrayList<String> rejectPermissions, @NonNull ArrayList<String> invalidPermissions) {
        //可选自行实现  权限未同意
    }
    ...
    public String BuildTips(boolean isCancel, boolean notEmpty, int countX, @NonNull ArrayList<String> permissions, boolean isFinal) {
        String names = TPermission.QueryNames(permissions);
        ...
        __execTipsWrap(false, notEmpty, baseCount + (res.IsFinalReject ? 1 : 0), permissions, res.IsFinalReject,
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        pItem.Handle.Request(RxPermission.this, permission, new RequestCallback() {
                            @Override
    ...
    private void __execFinalGrant(int baseCount, final ArrayList<String> permissions, final Runnable run) {
        ...
        __execTipsWrap(false, false, baseCount + 1, permissions, true
                , new Runnable() {
                    @Override
                    public void run() {
                        Settings.OpenSettings(ThisActivity, null, new OpenSettingsCallback() {
                            @Override
                            public void onResult() {
                                run.run();
                            }
        ...
        __execTipsWrap(false, false, baseCount, permissions, false
                , new Runnable() {
                    ...
                    public void run() {
                        ...
                        RequestFragment.RequestPermissions(ThisActivity, permissions, new RequestPermissionsCallBack() {
                            @Override
    ...
    private void __callback(@NonNull ArrayList<String> rejectFinalPermissions, @NonNull ArrayList<String> rejectPermissions) {
        ...
        if (rejectPermissions.size() == 0 && _invalidPermissions.size() == 0) {
            onPermissionSuccess(_lowerPermissions);
        } else {
            onPermissionFailure(rejectFinalPermissions, rejectPermissions, _invalidPermissions);
